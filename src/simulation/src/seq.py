...
if __name__ == '__main__':
    try:
        ...
        goal.target_pose.pose.position.x = 6.23
        goal.target_pose.pose.position.y = -4.37
        ...
        goal.target_pose.header.frame_id = "map"
        goal.target_pose.header.stamp = rospy.Time.now()
        goal.target_pose.pose.position.x = -6.12
        goal.target_pose.pose.position.y = -6.05
        goal.target_pose.pose.orientation.w = 1.0
        result = movebase_client(goal)
        if result:
            rospy.loginfo("Goal execution done!")
        ...
        goal.target_pose.pose.position.y = 7.18
        goal.target_pose.pose.orientation.w = 1.0
        result = movebase_client(goal)
        if result:
            rospy.loginfo("Goal execution done!")
